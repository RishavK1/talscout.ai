...
/**
 * Blueprint AI adapters via Perplexity Sonar (PERPLEXITY_API_KEY_PRIMARY) —
 * the PRIMARY tier ahead of Gemini/OpenRouter (see container.ts). Same
 * prompts/anti-hallucination discipline as the Gemini versions; website text
 * is still treated strictly as UNTRUSTED DATA.
 */
...
export class PerplexityBlueprintResearcher implements BlueprintResearcher {
  async suggest(args: { websiteUrl: string; name: string }): Promise<BlueprintSuggestions> {
    ...
    try {
      return await callPerplexityWithFallback<BlueprintSuggestions>({
        systemPrompt: RESEARCH_SYSTEM_PROMPT,
        userContent,
        temperature: 0.4,
        parse: (raw) => {
          const parsed = parseJsonLoosely<BlueprintSuggestions>(raw);
          if (!parsed.fields?.length) throw new Error("Perplexity blueprint suggestions missing fields");
          return {
      ...
      // Fail soft, same as every other tier: a usable wizard beats a dead
      // end. The caller (FallbackBlueprintResearcher) will actually try
      // Gemini/OpenRouter before ever reaching this catch in practice —
      // this only fires if Perplexity is used standalone.
...
export class PerplexityBlueprintGenerator implements BlueprintGenerator {
  async generate(input: BlueprintIntakeAnswers): Promise<BlueprintSections> {
    ...
    return callPerplexityWithFallback<BlueprintSections>({
      ...
      parse: (raw) => {
        ...
        if (!parsed.whoWeAre || !parsed.whatWeOffer) {
          throw new Error("Perplexity blueprint generation missing required sections");
        }
